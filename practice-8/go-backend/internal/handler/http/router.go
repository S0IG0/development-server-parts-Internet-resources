...
import (
	_ "Cookie/api/swagger"
	"Cookie/internal/service/encryptservice"
	"Cookie/pkg/logger"
	"Cookie/pkg/middleware"
...
func (r *Router) WithHandler(h HandlerRouter, logger logger.Logger) *Router {
	...
	api.Use(middleware.CookieMiddleware(
		logger,
		encryptservice.NewEncryptionService("YourSecretKey123"),
	))
